...
class IntentAgent(BaseAgent):
    ...
    async def get_intent(text: str) -> str:
        messages = [
            SystemMessage(
                content="Ты четко определяешь только intent запроса на русском и делаешь это не более чем за 5 слов. Никогда не пиши ничего кроме intent. Intet - это предмет того, что хочет пользователь или с чем возникла проблема. Ты работаешь в комании МТС, так что часто запросы пользователя буду связаны с нашими сервисами."
            ),
    ...
    async def extract_entities(intent: str, text: str) -> str:
        messages = [
            ...
            HumanMessage(
                content=f"Твоя задача: получить сущности, нужные для работы с {intent} из текста {text}"
            ),
        ]
        response = await chat_model.apredict_messages(messages)
        return response.content
    ...
    async def analyze_user(user: str) -> str:
        messages = [
            SystemMessage(
                content="ты ищешь зацепки о финансовом положении пользователя и его лояльности"
            ),
            HumanMessage(
                content=f"Проанализируй информацию и определи кейсы {user}. Напиши только самый правдоподобный"
            ),
        ]
        response = await chat_model.apredict_messages(messages)
        return response.content
    ...
    async def summ_ask(intent: str, entitiesstr: str, user_info: str | None) -> str:
        message = [
            SystemMessage(
                content="Проанализировав все свойствыа ты даешь краткую точную информацию зачем пользователь написал в поддержку на основании данных про него исключительно итоговую оценку"
            ),
            HumanMessage(
                content=f"Проанализируй {intent}, {entitiesstr}, {user_info} "
            ),
        ]
